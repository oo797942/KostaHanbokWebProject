...
<% String projectName = "/HoProject"; 
	...
	Object obj = request.getAttribute("MyBoardList");
	List<HoBoard> list = null;
	if(obj!=null) list = (List<HoBoard>)obj;
...
<body>
	...
	<table class="table table-bordered"  border="2" width="500" height="105">
		...
		<tr>
			...
			<td class="td1" valign="middle" height="50">내용</td>
			<td class="td1" valign="middle" height="50">답변상태</td>
		</tr>
		
		<%for(HoBoard board : list){ %>
		<tr>
		<td><a href="<%=projectName%>/..ho?cmd=Mylist-view&id=<%=board.getBoardNo()%>"><%=board.getBoardTitle() %></a></td>
		<td><a href="<%=projectName%>/..ho?cmd=Mylist-view&id=<%=board.getBoardNo()%>"><%= board.getBoardContent() %></a></td>
 		<td><a href="<%=projectName%>/..ho?cmd=Mylist-view&id=<%=board.getBoardNo()%>">
